...
- Templates and state layout:
  - Templates, config, and user templates are now stored under a project-local `SNID_SAGE` folder rooted at the directory where SNID SAGE is first run (e.g. `C:\path\to\project\SNID_SAGE\templates`), unless overridden with `SNID_SAGE_TEMPLATE_DIR` or `SNID_SAGE_STATE_DIR`.
